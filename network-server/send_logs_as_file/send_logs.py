...
def send_log(host, json_file, output_file):
    try:
    ...
    for log_entry in data:
        ...
        try:
            ...
            # Create a syslog-like message format
            formatted_message = f"{log_message}"

            # Save the formatted log message to a file in ECS format
            ...
            try:
                sock.sendto(json.dumps(log_entry).encode(), (host, 514))
...
if __name__ == "__main__":
    ...
    output_file = "./formatted_logs.txt"  # File to store logs in desired format

    while True:
        for json_file in json_files:
            send_log(elk_host, json_file, output_file)
